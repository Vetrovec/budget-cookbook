...
router.get('/', async (req, res) => {
	...
	if (maxPrice) {
		if (!maxPrice instanceof Number) {
			res.status(400).json(
				error.formatHttpError("Wrong maxPrice", "err-wrong-max-price")
			);
			return;
		}
...
router.post('/image/:id', async (req, res) => {
	...
	if (!image) {
		res.status(400).json(
			error.formatHttpError("Image is missing", "err-missing-image")
		);
		return;
	}
	...
	if (!/^image/.test(image.mimetype)) {
		res.status(400).json(
			error.formatHttpError("File is not image", "err-not-image")
		);
		return;
	}
...
router.post('/', async (req, res) => {
	...
	if (!recipe) {
		res.status(400).json(
			error.formatHttpError("Missing recipe", "err-missing-recipe")
		);
		return;
	}
	...
	for (const ingredient of ingredients) {
		if (!ingredient.id || !ingredient.amount) {
			res.status(400).json(
				error.formatHttpError("Invalid ingredients", "err-invalid-ingredients")
			);
			return;
		}
	}
	...
	if (!name || !name instanceof String) {
		res.status(400).json(
			error.formatHttpError("Invalid name", "err-invalid-name")
		);
		return;
	}
	...
	if (!duration || !duration instanceof Number) {
		res.status(400).json(
			error.formatHttpError("Invalid duration", "err-invalid-duration")
		);
		return;
	}
	...
	const fetchedIngredients = await ingredientDao.getByIds(ingredients.map((i) => {return i.id}));
	if (fetchedIngredients.length != ingredients.length) {
		res.status(400).json(
			error.formatHttpError("Non existent ingredient", "err-non-existent-ingredient")
		);
		return;
	}
